...
| league | `/league`, `/team/<id>` | League Hub (L1): grid de 12 times com cap/picks/dynasty/record + detalhe por time (roster, picks, cap breakdown). **L3:** cap **projetado** da season seguinte ao lado do corrente nas duas telas — agregado no render server-side via `routes.salary.compose_budget` (**zero query nova**: opera sobre os players já carregados), over-cap projetado destacado, selo PROV herdado do gate do Bid Máximo, e exibição **gated por `rollover_done`** (`_projection_open()` — pós-rollover a projeção significaria season+2). ⛔ **O Bid Máximo NÃO muda de base** (corrente = o mesmo número da keeper sheet) |
