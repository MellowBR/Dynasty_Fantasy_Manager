...
# Run cap projetado das telas de liga (L3 — 22 testes; composição única + guardas)
python cap_projetado_test.py
...
# Run busca de jogador (M10 — 27 testes; núcleo puro + endpoint em ORM de memória + guardas)
python player_search_test.py
...
| roster | `/`, `/player/<id>`, `/api/player/search` | Team rosters, cap bar, página dedicada por jogador (M13; O2-B1: header com time NFL + idade, link p/ `/team/<id>`, card de depth chart via `nfl_context.py` — pool lido do cache F13 **sem rede no caminho de página**, degradação silenciosa p/ dado ausente), banner de cap estourado em offseason (M1), banner informativo de IR (OFF26-16 — leitura, não controle). **IR-CLEANUP: não há toggle de IR** — o Sleeper é autoridade sobre `is_on_ir` e o sync sobrescreve. **M10:** `/api/player/search` é o backend ÚNICO da busca (navbar desktop+mobile e autocomplete da calculadora), servido pelo componente `createPlayerSearch` de `base.html` — payload `to_search_dict()`, ordem prefixo-primeiro, curingas do LIKE escapados. **M21-A:** FAs da liga (dropados) ENTRAM no resultado com badge "FA" no lugar da franquia, rosterado antes de FA no desempate; perfil de dropado sem botão de trade, salário rotulado "Último salário (histórico)" e tag IR suprimida (`is_on_ir` stale — sync intocado) |
| salary | `/salary`, `/salary_history`, `/cap_projector` | **L3: `compose_budget` (fonte única da composição "salário-base → roster sintético → `draft_budget`") vive aqui** — era inline no POST `/budget`; hoje serve também `/league` e `/team/<id>`. Salary calculator **com autocomplete de jogador (M10 — preenche ESPN/ano/aquisição; ⛔ o campo ESPN é RAW e o banco é ajustado: o autofill divide por 1.2, e o valor do ano 1 NÃO é preenchido)**, cap projector, salary history com timeline clicável. **DP1/DP2/DP3:** board de planejamento de rookie draft no cap_projector — lista a **classe entrante capturada** (`RookieEspnValue.in_class=True`, snapshot da captura admin DP3, menos já-rosterados; valorados ESPN no topo, massa a $1 atrás de busca/filtro) via `/api/cap_projector/rookies`; cenário keep/corte + rookies num único POST ao `/budget` canônico (DP2 — o antigo `/simulate` foi removido), projeção pura sem escrever contrato |
...
| league | `/league`, `/team/<id>` | League Hub (L1): grid de 12 times com cap/picks/dynasty/record + detalhe por time (roster, picks, cap breakdown). **L3 + L3-FIX:** o card é **ferramenta de planejamento da auction**, em 3 zonas — (1) bloco de destaque com **bid máximo projetado `<ano>`** (maior número), **cap projetado** e **slots livres**; (2) linha discreta **"Atual: cap · bid"**; (3) rodapé picks/record/dynasty. Pós-rollover o gate fecha e **as grandezas correntes assumem o mesmo bloco** (macro Jinja `bloco_destaque`, dois usos — nenhuma condicional além do gate). Todo projetado vem de `routes.salary.compose_budget` (**zero query nova**: opera sobre os players já carregados); `slots` = `empty_spots` do mesmo `draft_budget` do bid. PROV nas **duas** grandezas projetadas (é o que explica os pares coincidentes sob ESPN provisória); over-cap projetado destacado. Gate = `_projection_open()`/`rollover_done` — pós-rollover a projeção significaria season+2; ⚠️ **nada regrava a flag para `"false"`**, a reabertura no ciclo seguinte é o item **L4**. `/team/<id>`: "Cap atual"/"Resto atual" × "Cap proj. `<ano>`"/"Resto proj.". ⛔ **O Bid Máximo ATUAL não muda de base** (corrente = o mesmo número da keeper sheet e o oficial da auction); o projetado é campo separado |
...
⛔ **A busca do M10 (`/api/player/search`) não é um caminho de resolução** e não usa este módulo: o `ilike` substring de lá produz **sugestão exibida** (com posição + time NFL + franquia para desambiguar homônimos), e quem resolve é o clique do usuário — o destino sai do `id` da linha, nunca do nome. As duas coisas coexistem de propósito: reconciliação de import é estrita; autocomplete é permissivo **e não decide**.
...
fantasy_manager/
  ...
  cap_projetado_test.py             # L3: cap projetado da /league e /team/<id> (22) —
                                    #   composição única + gate de fase + ⛔ Bid Máximo intocado
