...
> 📁 Entradas anteriores em **`improvements_sessions.md`** (66 fechamentos, movidos verbatim — MAN-UX10-UX11-REG, MAN-UX12-REG/F1/REFINE, MAN-O2-F2-B1).
...
| O2 | Enriquecer página do jogador: contexto NFL (time + **link do time da liga** + **idade** + depth chart — 7 dimensões pós-roteamento (b) do [[UX12]]) + stats históricas + ECR/ADP + schedule — MAN-O2-F1/MAN-UX12-REFINE/**MAN-O2-F2-B1** | Média | ⚠️ **Batch 1 implementado 08/08/2026** (`nfl_context.py` núcleo puro + 19 testes; página nunca faz rede — pool via cache F13, stale-while-usable, zero cache novo em disco; idade derivada, não persistida; smoke local DJ Moore + DEF + fora-do-pool OK) — **falta smoke em produção (PROC1)**; Batch 2 (stats + schedule) pendente |
...
⚠️ **Batch 1 IMPLEMENTADO 08/08/2026 (MAN-O2-F2-B1) — aguardando smoke em produção (gate
[[PROC1]]); Batch 2 (stats históricas + schedule) segue pendente** — Prioridade **Média** —
...
#### F2 Batch 1 — entregue (MAN-O2-F2-B1, 08/08/2026; smoke prod pendente)
