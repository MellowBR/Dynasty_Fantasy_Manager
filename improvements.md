...
| OFF26-1 | Janela de cortes selada no Manager (declaração privada de cortes + budget ao vivo não-projetado + lock/revelação simultânea admin-manual, snapshot M8) — MAN-OFF26-REG/F1/REFINE/F2/SMOKE/**ENSAIO**/**ETAPA2** | Alta | ⚠️ **MECANISMO PROVADO EM PRODUÇÃO — Etapa 2 do ensaio (06/08): ciclo completo com 12 declarações de contas distintas, manter-todos e hierarquia owner>admin exercitados, sigilo cruzado, lock+reveal, hash `52274d01…`, sheet, reset limpo (0/0/fechada) e janela reabrindo; backup `/data/pre_ensaio_off26_1.db` (618.496 B)**. Etapa 1 (localhost, 06/08) 10/10. **REDESENHO: a porta de declaração do owner foi APOSENTADA** (cortes de 20/08 vão para o Sleeper; Manager só fotografa — ver OFF26-10); motor legado mantido para admin. **⚠️ só por causa do smoke do código desta sessão (PROC1)** |
| OFF26-2 | Keeper sheet consolidada (12 times; keeper+salário+**IR**+Bid Máximo, tabela+CSV) — insumo do Cowork — MAN-OFF26-REG/F1/REFINE/F2/SMOKE/ETAPA2/**MAN-OFF26-10** | Alta | ⚠️ **ORIGEM REESCRITA (U7): a sheet nasce do SYNC — keepers = roster vivo, sem gate de snapshot** (a janela extinta não produz mais nenhum). Provisória × definitiva pelo **carimbo do sync** (definitiva = sync POSTERIOR à revelação da urna). Sheet virou artefato **@admin_required** (decisão do owner: transcrição/auditoria, não consulta de owner). O ⚠️ de FONTE está resolvido; resta o smoke em prod ([[PROC1]]) |
...
| OFF26-10 | **Late drop = a URNA** — F2 ENTREGUE (07/08). Tabelas próprias (`LateDropDeclaration`/`LateDropAudit`), blueprint `late_drop` em **`/late_drop`**, ⛔ **flag de estado própria** (`late_drop_opens_at/closes_at` — reusar `cuts_window_open` reabriria a porta antiga; há teste que falha). U1 lista de **escolha única** (radio com cara de checkbox + "não vou dropar ninguém" como item) · U2 silêncio=passo · U3 horário do admin · U4 substituível · U5 lock/hash/reveal **reusando `compute_cut_snapshot_hash`** · U6 elegibilidade + flag de rookie de 1ª **OFF por default** · U7 sheet via sync · **U-CONF confirmação inline** · hierarquia owner>admin. Revelação produz a **lista de drops a executar**; execução **manual** no Sleeper — MAN-OFF26-10-11-REG → -SPEC → -ETAPA2 → **MAN-OFF26-10** | Alta | ⚠️ **+AJUSTES (07/08): contagem agregada "N/12" de volta (drop e passo contam igual — nem inclinação vaza; agregado devolve números, não times) e **bloqueio mútuo urna × rollover em código** nos dois sentidos (bilhete é escopado por season: virar a season no meio deixaria a revelação vazia, sem erro), com escape pelo banner de ensaio para o smoke. **64 testes + E2E 42/42; aguarda smoke em prod** (runbook `runbook_urna_late_drop.md`) |
...
| OFF26-15 | **Keeper sheet não marcava quem está em IR** — keeper em IR ocupa designação no board, e omiti-lo o **expõe ao leilão** — MAN-OFF26-14-F2 → **MAN-OFF26-10** | Alta | ⚠️ **FECHADO junto com a reescrita da sheet (U7)**: cada keeper carrega `is_on_ir`, a tabela tem coluna **IR**, o CSV tem coluna `IR` e o cabeçalho do time mostra **"N no IR"**. Medido no dry-run: **5 linhas marcadas**. Pendente de smoke em prod |
...
⚠️ **MECANISMO SELADO PROVADO EM PRODUÇÃO (Etapa 2 do ensaio, 06/08/2026 — ciclo completo,
12 declarações, hash `52274d01…`, reset limpo). O que mantém ⚠️: o código desta sessão
(aposentadoria da porta de declaração, MAN-OFF26-1-ETAPA2) ainda não foi smokado em prod
([[PROC1]]) — e só isso** — Prioridade **Alta**
...
⚠️ **ORIGEM REESCRITA E ENTREGUE (07/08/2026, MAN-OFF26-10 · U7): a sheet nasce do SYNC —
`keepers = roster vivo`, sem gate de snapshot. O ⚠️ de FONTE está resolvido; resta o smoke em
produção ([[PROC1]])** — Prioridade **Alta**
...
⚠️ **F2 ENTREGUE + AJUSTES (07/08/2026, MAN-OFF26-10 → -AJUSTES) — 64 testes + dry-run E2E 42/42;
aguarda smoke em produção ([[PROC1]], runbook `runbook_urna_late_drop.md`)** — MAN-OFF26-10-11-REG → -SPEC →
-ETAPA2 → **MAN-OFF26-10** — Prioridade **Alta** (caminho crítico **20/08 e 22/08**)
...
⚠️ **RESOLVIDO em 07/08/2026 (MAN-OFF26-10), junto com a reescrita da sheet — aguarda smoke em
produção** — Prioridade **Alta** — achado da [[OFF26-14]] F1, escopado como item na F2 (04/08/2026)
