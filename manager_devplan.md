...
### MAN-OFF26-11-F2 — o importador ingere só arremates: a keeper sheet CONGELADA como lista de exclusão (08/08/2026, Opus)

**O item fecha o último buraco de código do caminho crítico de 24/08.** A decisão de produto já
estava arbitrada (06/08, opção A — Manager é fonte única). O que esta sessão decidiu foi **como**.

- **O discriminador nasceu em módulo próprio (`keeper_exclusion.py`), não dentro do importador.**
  Núcleo puro (`build_index`/`classify_pick`/`compute_exclusion_hash`) + IO, no molde do
  `salary_engine`/`keeper_audit`. Regra única: **pick cujo jogador consta na lista para o MESMO
  time do pick é keeper**. A lista vem dos **dois produtores já existentes** da sheet — nenhuma
  segunda definição de "quem é keeper".

- **A decisão de peso: CONGELAR a lista, e por que.** A sheet nasce do **roster vivo**, e depois do
  leilão cada owner adiciona seus arremates **na liga real**. Derivar a exclusão ao vivo no import
  faria o arremate readicionado aparecer como keeper — e ele seria **excluído da ingestão**. Isso é
  o **dano invertido**, simétrico ao que o item existe para evitar: em vez de zerar a idade de um
  contrato, o contrato ano 1 simplesmente **não nasce**. O congelamento é ato explícito de admin,
  entre o sync final e o leilão, com hash e carimbos. Descartadas: derivar ao vivo (é o bug), gatear
  por carimbo de sync (recusa o import justamente quando ele é correto), snapshotar automático no
  1º preview (congela sem que ninguém tenha declarado o momento).
  **O que não cobre, declarado:** congelar **tarde** — mitigação é operacional, e por isso o passo
  entrou no runbook com hora marcada.

- ⛔ **Premissa do prompt que CAIU: o caso canônico não era ingerível pela UI.**
  `find_player_by_sleeper_id` filtra `is_dropped=False` → o jogador dropado na janela cai em
  *unmatched* ("jogador dropado no banco"), e o `<select>` só oferecia **Pular** e **Criar novo**.
  A API já aceitava resolver para um `player_id` existente; **nada na tela expunha isso**, e "criar
  novo" duplicaria o Player com o mesmo `sleeper_id`. Sem corrigir, a validação central do item
  ($50 dropado → recomprado → **ano 1**) era impossível de executar. Corrigido com
  `suggested_player_id` no preview + ação *"Reativar &lt;nome&gt; (ano 1)"*.
  **Padrão recorrente:** a capacidade existia no backend e morria na tela — mesma família do
  U-CONF (o `confirm()` que barrou uma declaração no celular).

- **Achado lateral virou item ([[OFF26-22]] 🔲 Média):** `keeper_audit.build_sheet` ainda tem
  `if not raw.get("revealed")`, mas o U7 fez `_build_keeper_sheet` devolver `revealed: True`
  **sempre**. O ramo é código morto — e com ele morreu o único bloqueio da auditoria por falta de
  insumo: **o gate do leilão hoje audita sheet PROVISÓRIA como se fosse definitiva**. O estágio
  está calculado; `build_sheet` só o descarta. Não tocado por estar sob restrição desta F2.

- **Réplica de leitura de picks: 3 sítios, 3 coerções do lance** (`float`/1.0 no importador,
  string→`_to_int`/0 na auditoria, `int`/None no sync). Já era o achado P6 do probe de 03/08 e
  **segue aberto** — unificar mexeria em sync e auditoria, ambos fora do escopo. O fix do
  discriminador **não precisa** alcançá-los: nenhum dos dois cria contrato.

- **Dupla contagem latente fechada de graça:** `_budget_alerts` somava `matched` inteiro, e o
  keeper já está no roster corrente que serve de base — no caminho novo ele contaria **duas vezes**.
  Com a exclusão, a soma passa a ser só de arremates, sem tocar a fórmula.

- **Confirmação empírica oportunista (só `GET`):** board da fantasma ainda populado — `draft_id`
  derivado do `league_id`, **24 designações**, totais **$148/$95/$60**, **`is_keeper: false` em
  24/24**. ⛔ **Draft em `pre_draft`: o pós-draft segue NÃO OBSERVADO**, e não foi forçado (rodar o
  draft é escrita na plataforma, ato do owner). O discriminador **não lê** o campo — há teste que
  falha se a string aparecer no módulo.

- **Regressão do modo linear provada, não argumentada:** preview do rookie 2025 real
  (`1224848075617484800`) sobre cópia temporária do banco, comparando o código novo com o **HEAD
  carregado do git** — dicionários **idênticos** campo a campo (33 matched / 3 unmatched / causas /
  salários / alertas), 0 escritas.
...
### MAN-UX10-UX11-REG — dois itens de UI registrados (08/08/2026, Opus) · docs-only

Sessão de **registro puro**, sem código e sem diagnose. Dois sintomas reportados pelo owner, nenhum
no caminho crítico de 24/08.

- **[[UX10]] 🔲 Baixa — fotos de jogadores desatualizadas.** Alguns jogadores exibem a foto da
  temporada anterior; o caso-âncora dado pelo owner é **David Montgomery**. Cosmético: a identidade
  é resolvida por `sleeper_id`, então nenhum dado de salário/contrato/roster é afetado — o custo é
  de leitura, porque o owner reconhece jogador pela foto. **A F1 foi registrada com as três
  hipóteses explicitamente NÃO arbitradas** (cache do navegador/CDN × URL construída com componente
  desatualizado × fonte keyed por algo que não o `sleeper_id`), porque elas levam a fixes
  **incompatíveis** — escolher errado produz correção que não muda nada ou que mascara o sintoma.

- **[[UX11]] 🔲 Média — quadro de trades sem a franquia NFL do jogador.** Informação usada para
  avaliar a troca (bye, backfield, contexto ofensivo) que hoje o owner busca fora da tela. Sem
  prazo.

- **Padrão que já entra no registro, não só no fix:** os dois itens carregam a **pergunta de
  réplica** que virou rotina nas últimas sessões — a construção da URL de foto existe em mais de um
  lugar (templates/JS/Python)? a exibição de time do jogador tem fonte comum ou **cada tela deriva a
  sua**? A segunda tem consequência imediata de desenho: se cada tela deriva, acrescentar mais uma
  no quadro de trades **piora** o problema em vez de resolvê-lo.

- **Raiz comum possível, registrada sem afirmar:** a hipótese (b) do UX10 (componente de time
  desatualizado na URL) e a pergunta 2 do UX11 (o dado de time está atualizado?) podem ser o mesmo
  problema visto de dois ângulos. Fica anotado como suspeita, não como conclusão.
...
### MAN-UX12-REG — busca de jogador + perfil enriquecido, e a sobreposição que o registro expôs (08/08/2026, Opus) · docs-only

Sessão de **registro puro**, sem código e sem diagnose. Pedido do **co-admin Michel**, trazido pelo
owner — e a procedência importa: o Michel é quem faz o smoke das features user-facing, então o
pedido nasce de uso.

- **[[UX12]] 🔲 Média — busca de jogador + página de perfil enriquecida.** São **dois** sintomas, e o
  segundo só aparece depois de resolver o primeiro: (1) **não existe caminho de busca** — o único
  acesso a um jogador é clicá-lo dentro da página de um time, ou seja, é preciso **já saber em que
  franquia ele está** para conseguir abri-lo; (2) a página que abre (a `/player/<id>` do [[M13]] ✅)
  não traz o que a liga usa para avaliar o jogador. Sete campos pedidos: time NFL + franquia da liga,
  link p/ a página do time, link p/ trade **com o time dele já pré-selecionado**, cap hit, idade,
  **depth chart na NFL** e histórico de `PlayerHistory`.

- **O achado desta sessão não foi o item — foi a sobreposição, e ela ficou explícita.** O escopo
  pedido atravessa **dois itens 🔲 que já estão no backlog**: [[M10]] **é** a busca (refinado em
  28/04/2026, com o endpoint `GET /api/player/search` **já existente**, os 5 entry points mapeados e
  um caso de uso registrado — *"queria ver o contrato do Mahomes e teria que abrir os 12 rosters"* —
  que é **o mesmo sintoma** que o Michel relata agora, dois usuários diferentes com três meses de
  intervalo); e [[O2]] **é** o perfil, já incluindo nominalmente **time NFL no header** e **depth
  chart**. O que o UX12 acrescenta de novo: os campos **2, 3, 5 e 7**, o agrupamento como uma
  experiência só, e a **demanda de um segundo usuário** — que é validação independente de
  prioridade, não item novo por si.

- **Roteamento NÃO arbitrado, e registrado como questão 0.** Três caminhos ficam abertos para a F1:
  (a) UX12 vira **guarda-chuva** e absorve M10/O2; (b) UX12 é **despachado** (busca → M10, perfil →
  O2) e fecha como registro de demanda; (c) UX12 fica só com **o que sobra**. ⛔ **Implementar em
  paralelo a M10/O2 está fora** — seria a enésima réplica, exatamente a classe de problema que as
  últimas sessões vêm fechando. O precedente de método é do próprio backlog: o M10 foi **refinado
  in-place** com o ID preservado, e o [[UX4]] foi **consolidado** no O2 em vez de duplicado.

- **Questões abertas registradas, nenhuma respondida.** **Réplica de fonte** do time do jogador — a
  mesma pergunta central do [[UX11]], com a relação registrada dos dois lados (a F1 do UX12 pode
  absorver ou despachar o UX11). **Disponibilidade do dado**: o sync persiste **depth chart** e
  **idade** hoje? ⛔ a F1 **confere** em vez de herdar a afirmação do O2 de que `depth_chart_order`
  já é consumido — e, se não persiste, pesa persistir × derivar em leitura (o cache tem ~15 MB e
  vive fora do git, [[F13]]). **Página nova × enriquecer a existente**, com os links internos que
  apontam para a atual. **Busca**: superfície (navbar × página dedicada) e homônimos — ⛔ exibição
  pode usar nome, **identidade é por `sleeper_player_id`** (precedente Brown; `player_lookup.py` é
  estrito de propósito e **não serve** para autocomplete, como o M10 já registrava). **Foto**: se
  entrar no perfil, herda o [[UX10]] — cross-ref **sem acoplar** (o UX12 não espera o UX10).

- **Duas guardas de invariante já anotadas no registro**, para a F1 não precisar redescobri-las: o
  **cap hit** exibido não pode virar uma sétima definição de folha ([[OFF26-16]] — régua única, o IR
  conta), e o **histórico** do campo 7 esbarra no [[S4]] 🔲 (`PlayerHistory` identifica time **só por
  nome**).
...
### MAN-UX12-REFINE — roteamento (b) executado: despacho em M10/O2, UX12 fechado como roteado (08/08/2026, Fable) · docs-only

O owner confirmou a recomendação da F1 e esta sessão executou o despacho completo. Zero código;
nenhum status mudou além do UX12 (🔲 → ✅ ROTEADO).

- **[[O2]] refinado in-place — o grosso da sessão.** (1) Absorveu os campos **2** (link p/ a
  página do time na liga — o nome no header é `<strong>` puro, vira link p/ `/team/<id>`) e
  **5** (idade — inexistente no Manager; `birth_date` no pool com 94%, imutável — persistir tem
  zero staleness, derivar custa zero schema; escolha na F2), juntando-se aos campos 1 e 6 que o
  item já cobria — o Batch 1 passa a entregar o contexto NFL inteiro, caso DJ Moore **e** pedido
  do Michel. (2) **Dívida de absorção quitada:** a F1 consolidada do MAN-O2-F1 (28/04/2026) saiu
  do `handoff_code_manager_28_04_2026_pt2.md` — arquivo que se autodeclara descartável — e passou
  a viver na seção do O2 (tabela de disponibilidade por dimensão, endpoints corrigidos **sem
  `/v1/`**, batching em 2 batches, escopo das stats, fonte do schedule, cache strategy — com nota
  pós-F13: caches novos em `dirname(DYNASTY_DB)` com carimbo interno — e reuso mapeado). A regra
  é diagnose viver no `improvements.md`; era o momento de corrigir. (3) **Afirmação falsa
  corrigida no texto** (achado Q2 da UX12-F1): `depth_chart*` não tem consumo em produção — no
  lugar, o achado real (dado no pool, 75% skill; **coluna não bastaria**, rivais de posição não
  são Players do DB; derivação do pool, TTL 168h, stale conhecido out–nov). (4) **Guardas
  herdadas registradas:** cap hit só pelas fontes canônicas ([[OFF26-16]] — nenhuma definição
  nova de folha) e histórico exibindo `PlayerHistory` como está (nome como snapshot; zero
  acoplamento novo com [[S4]], cujo risco é de escrita). (5) **Decisão Q5 registrada:** enriquecer
  a página existente — 2 helpers + 6 links inline já apontam para ela.

- **[[M10]] — só o cross-ref de demanda, spec intocada.** Segundo usuário (Michel, 08/2026)
  relatando o mesmo sintoma do caso de uso original de 28/04 (“abrir os 12 rosters procurando”).
  A spec de 28/04 segue autoritativa — a UX12-F1 a conferiu inteira e viva.

- **[[UX12]] ✅ ROTEADO.** Sem escopo próprio remanescente: campos 2/5 → O2; 1/6 já eram O2;
  3/4/7 **já existem** na página atual (achado da F1); busca já era M10. Seção completa (registro
  + diagnose F1, a evidência do roteamento) migrada **verbatim** ao `improvements_archive.md`
  conforme [[O3]], conferida 1× lá / 0× no ativo; M10 e O2 apontam o archive como origem dos
  requisitos.

- **[[UX11]] reclassificado por transbordo (status segue 🔲).** A Q1 da UX12-F1 respondeu a
  F1 dele: o payload do quadro de trades (`/api/roster/by_name/` → `to_dict()`) **já contém
  `nfl_team`** — falta só renderizar. Vira **candidato a F2 direta de causa conhecida** (1 edição
  de template literal, carona em sessão futura), com a ressalva de staleness registrada (sync não
  limpa o time de quem vira FA — 17 casos medidos — e não roda em todo boot).

- **[[UX10]] estreitado por transbordo (status segue 🔲).** Hipóteses (b) e (c) **descartadas
  por evidência**: a URL de foto é keyed só por `sleeper_player_id`, sem componente de temporada/
  time, com exatamente 2 construtores espelhados e zero inline. Sobra a (a) — cache/CDN; a F1
  residual é confrontar o caso-âncora (David Montgomery) com o que o CDN devolve.

**Exceção de commit registrada (regra da checklist):** commit **docs-only**, deliberado — sessão
de roteamento documental, sem código pendente e sem nada a smokar. Validações: Status Rápido com
1 linha alterada e contagem preservada; seções UX10/UX11 com texto pré-existente byte-idêntico
(transbordos são appends); migração verbatim conferida.

### MAN-O2-F2-B1 — Batch 1 do perfil do jogador: contexto NFL derivado do pool (08/08/2026, Fable)

F2 da camada 1 do [[O2]], conforme a spec refinada em `3dde0ab`. As 4 dimensões do Batch 1
estão na `/player/<id>`; [[O2]] vai a **⚠️** — smoke em produção pendente (gate PROC1), Batch 2
(stats históricas + schedule) segue como escopo aberto.

- **O que entrou.** Header ganhou a linha `🏈 <time NFL> · <idade> anos` (time de
  `Player.nfl_team` — a **fonte única** mapeada na Q1 da UX12-F1; idade do pool); o nome do time
  da liga virou **link p/ `/team/<id>`** (rota L1 existente, `url_for` — nada hardcoded); card
  novo de **depth chart** do time NFL com o jogador destacado — match por **sid**, nunca por nome
  (há dois “DJ Moore” no pool real; o homônimo CB de 1987 não contamina).

- **`nfl_context.py` na mesma separação do salary_engine.** Núcleo puro (`build_slim_index`,
  `compute_age`, `build_depth_chart`, `assemble_context` — sem DB/rede/FS; `today` é parâmetro)
  + IO mínima. **19 testes** em `nfl_context_test.py` cobrem ordenação/desempate, degradações
  (sem ordem, sem birth_date, fora do pool, DEF), homônimos e normalização DST→DEF.

- **As três decisões de desenho, e por quê.** (1) ⛔ **O caminho de página nunca faz rede**: lê
  o cache do pool pelo caminho único do F13 (`sync_sleeper._player_cache_path`) e **não dispara
  download** — um GET de página não pode pagar 15 MB de fetch nem depender da Sleeper estar de
  pé; pool ausente → contexto vazio e página normal. Pool **vencido ainda serve**
  (stale-while-usable): chart de 8 dias > bloco sumindo, e o staleness já é ressalva registrada
  da seção. Renovar é papel do sync (TTL 168h). (2) **Nenhum cache novo em disco** — a nota
  pós-F13 satisfeita por vacuidade: índice enxuto (7 campos × ~12k entradas) em memória de
  processo, invalidado por `(mtime, size)` **como chave de invalidação, nunca como validade** (a
  lição do F13 — rebuild espúrio pós-deploy é inócuo). (3) **Idade derivada, não persistida**
  (a F1 deixara a escolha): zero schema, e calculada de `birth_date` — não do campo `age` do
  pool, que envelhece entre fetches.

- **Autoridades separadas, como a seção pede:** o time do **header** vem de `Player.nfl_team`
  (fonte única, escrita só pelo sync); o time do **chart** vem da entrada do jogador **no pool**
  (autoridade do depth chart). Divergência entre eles é o staleness documentado — não foi criada
  uma segunda derivação de exibição.

- **Smoke local (GET-only, test client logado).** DJ Moore (âncora): `🏈 BUF · 29 anos`
  (birth_date 1997-04-14 confere), chart WR do BUF com ele em **#1 destacado**, link para a
  franquia **dele** (`/team/4`) e não a do usuário logado (`/team/5`). DEF (Atlanta): sem card,
  sem erro. **3 rosterados fora do pool** (Gainwell, Hollywood Brown, Cameron Ward) — página
  normal sem idade/chart, sem erro: a degradação exigida pela spec foi exercitada com dado real.
  Nota: nenhum skill rosterado está hoje sem `depth_chart_order` (os ~25% sem o campo são fundo
  de elenco do pool) — o caminho “chart parcial” fica coberto pelos testes unitários.

- **Guardas e regressão.** Zero coluna nova; folha/cap intocados — `salary_engine_test` **54/54**
  (o prompt citava 48; a suíte tem 54 desde o OFF26-16); Timeline/`PlayerHistory` e rotas
  intactas. Suítes completas verdes: 34 (audit) + 25 (stage) + 36 (exclusion) + 64 (urna) + 22
  (ensaio) + 14 (régua) + 19 (novos). As 2 “falhas” do `janela_ensaio_test` no console são
  **artefato cp1252 pré-existente** (print de ⛔) — 22/22 com `PYTHONIOENCODING=utf-8`; nenhum
  arquivo desta sessão toca aquele caminho.
...
### MAN-M10-F2 — busca de jogador: global na navbar + autocomplete da calculadora (10/08/2026, Opus)

F2 do [[M10]] conforme a spec de 28/04 (conferida viva pela UX12-F1, `d72f378`) — **executada, não
redesenhada**. [[M10]] vai a **⚠️**: implementado, smoke de produção pendente (gate [[PROC1]]).

- **Uma engrenagem, dois consumidores.** `createPlayerSearch` (em `templates/base.html`) tem dois
  modos: **navegação** (sem `onSelect` → cada resultado é um `<a href="/player/<id>">` de verdade,
  ctrl/meio-clique funcionam) e **seleção** (`onSelect` → devolve o objeto e fecha). Três
  instâncias: navbar desktop, overlay mobile e calculadora. ⛔ Nenhuma tela deriva a sua — era o
  risco óbvio de um item com 2 consumidores.

- **Identidade, que é o que este item tinha de delicado.** O `ilike` do backend é **sugestão
  exibida**; a resolução é **sempre ato explícito do usuário**. Concretamente: linha traz
  **posição + time NFL + franquia** (o que separa homônimos), o destino sai do **`id` da linha**
  (nunca do nome), e **Enter sem item destacado não seleciona nada** — não existe "vai no
  primeiro". `player_lookup` não foi tocado nem chamado; segue sendo a reconciliação estrita dos
  imports, e há teste que falha se o nome dele aparecer no corpo do endpoint.

- **Backend reusado, 3 ajustes** (assinatura, `is_dropped=False` e teto de 20 preservados):
  (1) **`to_search_dict()`** — `to_dict()` invocava `is_renewal_candidate()` +
  `project_next_salary()` por jogador, ou seja **20 projeções de contrato a cada tecla** para
  exibir 4 campos; a spec deixara a otimização condicional, o custo real foi uma dúzia de linhas.
  (2) **Ordenação prefixo-primeiro** — sem `order_by`, o teto de 20 cortava em ordem arbitrária de
  tabela: em liga cheia, "moore" podia devolver 20 linhas **sem o Moore**. (3) **Escape dos
  curingas do LIKE** — `%` digitado devolvia o elenco inteiro rotulado como resultado de busca.

- **Questões que a spec delegou à F2, decididas:** breakpoint = o **768px que a navbar já usa**
  (nenhum novo); dropdown mobile em **fluxo normal** (empurra o painel — absoluto vaza num `aside`
  que rola); resultado renderizado no componente e **não** via `renderPlayerNameLink` (âncora
  aninhada é HTML inválido — mesmo destino, e a foto vem do `renderPlayerPhoto` existente, keyed
  por sid: as fontes comuns da Q1 da UX12-F1, sem derivação nova); os 2 consumidores numa camada
  só (com o componente pronto, o segundo custou o `onSelect`).

- ⚠️ **A conversão que era fácil de errar.** O campo "ESPN Ref Value" da calculadora é **RAW** (o
  `×1.2` está em `routes/salary.py`) e `Player.espn_ref_value` guarda o **ajustado**: o autofill
  **divide por 1.2**. Sem isso o valor seria multiplicado duas vezes e **inflaria a tabela de
  contrato inteira** — uma tela de projeção mentindo em silêncio. ⛔ E **"Valor Pago no Ano 1" não
  é preenchido**: o banco tem o salário corrente, não o do ano 1; preenchê-lo daria uma tabela
  plausível e falsa. As duas coisas viraram teste estático.

- **Regressão.** `player_search_test.py` **27** (escape do LIKE puro · endpoint contra ORM em
  memória: âncora Mahomes, **dois DJ Moore com ids/sids distintos e distinguíveis**, prefixo antes
  de substring, dropado fora, filtro por time, vazio gracioso, curinga, teto, payload sem projeção
  · guardas estáticas de identidade). Suítes completas verdes: 54 + 34 + 25 + 36 + 64 + 22 + 14 +
  19 + 27.

- **Smoke local (GET-only, test client logado, cópia do DB — o vivo não foi tocado).** "Mahomes"
  → 1 resultado com link direto: **o caso âncora de 28/04 morre**. "brown" → **5 Browns
  distinguíveis**, cada um com id próprio: o incidente vira *lista para escolher*, que é o
  comportamento pedido; de carona renderizaram sem erro os dois casos-limite reais — DEF com sid
  **sigla** ("CLE") e jogador **sem sid** (Hollywood Brown, foto omitida em silêncio).
  Inexistente/`%`/`_` → vazio. Componente exercido também em **DOM headless** (harness descartável
  em node, não versionado): debounce de 1 request, `<2` chars não busca, hrefs distintos por
  homônimo, Enter sem destaque não navega, seta+Enter vai ao id certo, 68.4 → 57.

- ⚠️ **O que NÃO foi exercido:** **interação em navegador** (aparência, foco/teclado reais,
  overlay em tela pequena) — é o smoke do owner. E o caso literal **"dois DJ Moore" é inexercível
  com dado real**: só o WR do BUF está rosterado, e a busca é do **DB local** por desenho da spec
  (que separou explicitamente a fonte da busca da fonte do [[O2]]); o cenário vive nos testes, e o
  análogo real em produção é o "brown".
...
### MAN-M21-REG — busca cobre o universo Sleeper: FAs da liga + não-rosterados (10/08/2026, Opus) · docs-only

Registro do pedido do owner feito **logo após o smoke do [[M10]]**: a busca deve alcançar qualquer
jogador do universo Sleeper, não só os rosterados. **Nenhuma diagnose, nenhuma decisão de desenho** —
o que esta sessão fez foi estruturar o pedido para que a F1 tenha o que responder.

- **O item nasce partido em duas fatias, e o corte é por NATUREZA, não por tamanho.** A **fatia A**
  (FAs da liga) é **um predicado de query** sobre dados que já existem: o jogador **é** `Player`,
  tem contrato histórico, `/player/<id>` abre normalmente — o único motivo de ele sumir é o filtro
  `is_dropped=False` da busca. A **fatia B** (rookies não-drafteados e o resto do pool) é **decisão
  de arquitetura de dados**, com potencial de mudar o significado da tabela `Player`. Compartilham
  superfície (mesma busca, mesma lista, mesma regra de identidade) — daí um ID só —, mas ⛔ **A não
  espera B**, e **B não se resolve de carona na A**.

- **Prazos próprios:** A é **Alta com alvo pré-24/08** — consultar contrato/histórico de FA é
  preparação da **FA auction**, e hoje exige saber o `id` e digitar a URL. B é **Média,
  pós-intertemporada**. Registrado também que **A não bloqueia nem é bloqueada pelo smoke pendente
  do M10**: o smoke valida o que já está em produção (`5e3c403`); a fatia A é escopo novo.

- **Um fato de código entrou no registro (e só ele):** o sync marca `is_dropped=True` para quem
  está num time do DB mas **não** no roster do Sleeper daquele time — que é exatamente o estado de
  um FA da liga —, e a busca filtra por esse campo. Isso é **mecanismo**, não diagnose: explica o
  sintoma sem propor fix. ⚠️ **Nenhuma medição de produção**: o `dynasty.db` do git é seed, e o
  número de dropados dele (37/281) entrou marcado como ordem de grandeza, não como dado de prod.

- ⚠️ **A arbitragem central da fatia B ficou EM ABERTO de propósito** — importar o pool como
  `Player` × **busca federada em duas fontes**. A hipótese do owner ("deveriam estar na nossa
  base") foi registrada como **premissa a testar, não como spec**, com o trabalho da F1b explicitado:
  mapear os **consumidores da tabela `Player`** (folha/[[OFF26-16]], keeper exclusion/sheet,
  `needs_review`, agregações do League Hub e do cap projector, portas de aquisição) e o que ~12 mil
  linhas sem contrato fariam com cada um, **contra** o custo e os limites da federação. O
  precedente citado é o Batch 1 do [[O2]], que **lê o pool sem persistir nada**.

- **Demais questões registradas sem resposta.** Fatia A: por que o filtro `is_dropped` existe (ele
  foi **herdado do endpoint pré-M10** junto com a assinatura — a razão viva, se houver, condiciona
  incluir-marcado × incluir-sem-distinção × toggle); o que o perfil de um FA oferece que precisa de
  ajuste (o botão de trade é a suspeita óbvia); staleness herdada do [[UX11]] **sem reabrir aquela
  decisão**. Fatia B: destino do clique num não-`Player`; colisão de identidade entre duas origens
  (⛔ resolução sempre por `sleeper_player_id`, precedente Brown); relação com
  [[DP1]]/`RookieEspnValue`, ⛔ **sem criar uma segunda definição de "quem é entrante"**.

- **Higiene do registro:** Status Rápido ganhou **exatamente 1 linha** (M21 🔲, prioridade por
  fatia); nenhum status existente alterado; nenhuma seção de outro item editada — os cross-refs
  ([[M10]], [[UX11]], [[O2]], [[DP1]], [[M13]]/[[M14]], [[OFF26-16]]/[[OFF26-11]]) partem **da
  seção nova**. **Exceção de commit docs-only logada.** Zero código.

### MAN-M21-F1a — diagnose da fatia A: o filtro nunca protegeu nada, e o desenho recomendado é incluir marcado (10/08/2026, Fable)

F1a read-only da fatia A do [[M21]] (`13c9c8c`). Toda evidência do HEAD e do histórico git; zero
mutação, zero chamada externa, zero query em produção. Item segue **🔲** — decisão do owner.

- **Q1 — a arqueologia DECIDE a pergunta central do registro.** O endpoint `GET /api/player/search`
  nasceu na v1.0 (`f2271ba`) **já com o filtro `is_dropped=False` dentro** — e uma busca em todos
  os commits de `templates/`+`static/` (`git log --all -S "player/search"`) encontra **uma única
  referência: o próprio M10**. O endpoint passou v1.0→M10 **sem nenhum consumidor**. As hipóteses
  do registro "evitar poluição da busca" e "proteger o Propor Trade" caem por **anacronismo**
  (não existia busca nem fluxo a proteger quando o filtro foi escrito); sobra **inércia**. Não há
  legado: a decisão de incluir FAs é uma decisão **só sobre os 2 consumidores do M10** — os demais
  sítios com `is_dropped=False` têm queries próprias e não passam pelo endpoint.

- **Q2 — perfil de FA campo a campo (tabela completa na seção).** O que o registro não sabia: o
  tratamento **já é parcial** — tag "Dropado" no header desde o M13 (`player_detail.html:35`),
  porque o sync **preserva `team_id`/`fantasy_team` ao dropar** (`sync_sleeper.py:324` só liga o
  flag). Vereditos: **enganosos** = botão "⇄ Propor Trade" **aparece para FA de outro time**
  (`can_propose_trade`, `routes/roster.py:414-418`, não checa `is_dropped` — e é inconsistente:
  FA do próprio time esconde o botão) e a label **"Salário atual"** sobre número histórico que não
  conta em folha nenhuma (`roster_salary` filtra dropados). **Precisa contexto** = "Contrato Ano
  X/4" e a tag IR (`is_on_ir` fica **stale** em dropado — o sync só reescreve o campo de quem está
  em roster, `sync_sleeper.py:290`). **Corretos** = franquia+tag no header, campos históricos,
  `nfl_team` (ressalva UX11, sem tratamento novo), Timeline, depth chart.

- **Q3 — recomendação (a): incluir sempre, marcado.** O argumento decisivo contra a (b) é um
  mecanismo que o registro omitiu: `fantasy_team_name` de dropado devolve **a última franquia** —
  sem badge, o FA *parece rosterado* na lista. O badge "FA" deve entrar **no lugar da franquia** na
  sub-linha do resultado. O toggle (c) mata o caso-âncora por default (quem procura o Helm não sabe
  que precisa ligá-lo). **FA na calculadora faz sentido** — simular o contrato de um alvo da
  auction é exatamente o uso pré-24/08. Custo real da (a): `to_search_dict()` **não expõe
  `is_dropped`** → +1 campo no payload + 1 condicional no `optionInner`. Opção de ordenação
  registrada p/ F2a sem decidir: rosterado antes de FA no mesmo grupo de prefixo (1 `case`).

- **Q4 — calendário.** O pico é real e chega **de uma vez**: cortes de 20/08 acontecem no Sleeper e
  viram `is_dropped` no sync seguinte. O teto de 20 por query segue válido (busca por substring,
  não listagem); o risco de FA acima de rosterado na ordem alfabética é mitigado pelo badge e/ou
  pela ordenação opcional. **Urna: zero interseção** — lista do roster com query própria
  (`late_drop.py:153`) e bilhete recusa dropado (`late_drop.py:176`). E o mundo já trata dropado
  como entidade recomprável: fluxo "Reativar (ano 1)" do importador (`draft_import.py:64-69`) —
  precedente de coerência a favor da inclusão.

- **Q5 — medição é do owner.** 2 SELECTs read-only prontos na seção, contra `/data/dynasty.db`
  (o VIVO — a régua seed ≠ produção). ⛔ Não executados nesta sessão. Nota que reforça a régua: **o
  seed local não contém Gunnar Helm** — o caso-âncora só existe no banco vivo, criado por sync.

- **MAN-METH-REG:** (a) 2 premissas contraditas/decididas — as hipóteses de proteção do filtro
  (anacronismo) e o "campo aberto" do perfil (tratamento parcial já existia); (b) 4 omissões com
  parecer — `team_id` preservado no drop (o argumento da Q3), `to_search_dict()` sem `is_dropped`,
  `is_on_ir` stale, "Reativar (ano 1)".
...
### MAN-OFF26-23-REG-F1 — rookie Ano 1 × rollover × passo 5: a ordem é disciplina, não código (10/08/2026, Fable) · docs-only

Sessão combinada REG + F1 read-only (precedente de diagnose pura) sobre a pergunta do owner a 7
dias do draft: **o rookie drafteado em 2026 permanece Ano 1?** Resposta: **sim, se — e somente
se — o import do draft acontecer DEPOIS do rollover.** Nenhuma mutação; o entregável é o roteiro.

- **Q1 — a ordem canônica existe no desenho, não no código.** O painel gateia até o passo 4
  (rollover exige passos 2+3 e é **once-only** — reexecução devolve 400) e a urna exige
  `rollover_done`. ⛔ Mas o **importador do draft (OFF26-3) não tem gate nenhum de rollover** — a
  ordem *rollover → import* é disciplina operacional. O calendário pressupõe; o código não exige.

- **Q2 — o filtro real da varredura, citado:** `Player.query.filter_by(is_dropped=False)`
  (`offseason.py:686`), **cega a contrato recém-aberto** — sem skip por `contract_start_season`.
  `apply_season_rollover` faz `next_yr = contract_year + 1` (`salary_engine.py:206`). Import antes
  do rollover ⇒ **classe inteira vira Ano 2 com valorização** no dia seguinte. Import depois ⇒
  Ano 1 até 2027. O once-only elimina a família "reexecução" — o risco é SÓ de ordem.

- **Q3 — prod: o rollover 2026 ainda não rodou (alta confiança, conferência pronta).** Indício
  forte: o smoke da urna (07/08) precisou do **escape do banner de ensaio**, que só existe para
  contornar `rollover_done` pendente (`rollover_blocks_urn`). Seed local concorda
  (`current_season=2025`). Shell check read-only na seção. Implicação: **a janela de risco está
  inteira à frente** — a semana 17→24/08 é exatamente onde a ordem importa.

- **Q4 — o passo 5 não gateia nada além de si.** Grep completo: `rookie_draft_done` só é lido pela
  UI do painel. **O plano "segurar o clique até pós-24/08" é validado** — e endurecido pela
  omissão (b)1: **o consumidor mais crítico do store é o PRÓPRIO import do draft**
  (`rookie_espn_adjusted` → `floor(ESPN×1,2)`); clicar o passo 5 antes de importar zeraria os
  salários da classe para $1, e **o clear não tem undo** (reverter o toggle não repopula).

- **Q5 — Gainwell é lente, não precedente direto:** mesma manifestação (`contract_year` um acima),
  **raiz distinta** — lá, semântica de canal de aquisição (free_agent recomeça contrato; acidente
  de vocabulário do F8); cá, ordem de fases. O fix de lá cobriu a raiz de lá; a daqui não tem
  guarda de código. Se o pior acontecer, a porta `contract_year_correction` (molde M2, auditável)
  é o caminho de reparo já construído.

- **O roteiro seguro (tabela completa na seção):** 17/08 draft no Sleeper, **sem importar**; 18/08
  na ordem ESPN definitiva → **rollover** → **import do draft**; agendar urna (gate passa);
  20-22/08 fluxo normal; 24/08 auction + import com passo 5 **ainda não marcado**; pós-24/08,
  passo 5 (clear agora inofensivo) e passo 7. **Não há conflito** entre "rookie Ano 1" e "store
  vivo até a auction" — ambos saem da mesma ordem. **3 pontos de não-retorno marcados:** import
  antes do rollover · passo 5 antes do import · clear sem undo.

- **METH-REG:** (a) "a ordem está garantida pelo sistema" contradita (gate no import = candidato a
  item, ⛔ não arbitrado); "segurar o passo 5 trava algo" refutada a favor do plano. (b) 4
  omissões — o import como consumidor crítico do store (a maior), once-only, urna exige rollover,
  ESPN precede rollover por construção.
